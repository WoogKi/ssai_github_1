...
def diagnose_company(company_id: int) -> DiagnosticResult:
    ...
    result = DiagnosticResult(sms=int(company_id))
    try:
...
def _render_table(results: list[DiagnosticResult]) -> str:
    rows = [
        "SMS | ERP 접속 | SSAI_ANALYTICS 존재 | SSAI_ANALYTICS SELECT | 최종판정 | 비고",
        "--- | --- | --- | --- | --- | ---",
    ]
    rows.extend(
        f"{item.sms} | {item.erp_connect} | {item.analytics_exists} | "
        f"{item.analytics_select} | {item.verdict} | {item.note or '-'}"
        for item in results
    )
    return "\n".join(rows)
...
def main() -> int:
    ...
    txt_path.write_text(_render_table(results) + "\n", encoding="utf-8")
