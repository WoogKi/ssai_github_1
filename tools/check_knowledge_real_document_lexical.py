...
def evaluation_cases() -> tuple[EvalCase, ...]:
    ...
    return (
        ...
        EvalCase("stock_ignores_io_profile", "현재고는 저장 io gu list를 적용하지 않는다", ("nlq-current-stock-contract.md",), ("3.2 저장조건과 재고 계산",)),
        EvalCase("current_table_isolation", "회사 채팅방이 다른 current table export cache 재사용하지 않는다", ("nlq-current-stock-contract.md",), ("2. 공통 원칙",)),
        ...
        EvalCase("dashboard_llm_role", "Dashboard LLM 숫자를 다시 계산하지 않는다", ("dashboard-lite-design.md",), ("1. 목적",)),
        ...
        EvalCase("spacing_variant", "회사 채팅방 current table export cache 재사용하지 않는다", ("nlq-current-stock-contract.md",), ("2. 공통 원칙",)),
