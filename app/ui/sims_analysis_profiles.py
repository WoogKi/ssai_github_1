...
def snapshot_grade_analysis_contract(action: str = "", columns: Iterable[Any] = ()) -> str:
    ...
    if action != "제품정보 조회" and not labels.intersection(relevant) and not any(label in action for label in relevant):
        return ""
    cost_columns = {"추정단위손익", "추정손익률", "추정기여금액"}
    cost_contract = ""
    if labels.intersection(cost_columns):
        cost_contract = (
    ...
    return (
        ...
        "- 품목손익등급과 품목기여등급은 서로 독립적인 대상제품 내 상대등급 A~E이며 A가 상위, E가 하위다. "
        "출고빈도등급과도 독립이며 등급만으로 확정 흑자/적자를 판단하지 마라.\n"
        "- 집계표에서 그룹 컬럼은 분류 기준이며 수량 등은 그 그룹의 측정값이다. "
        "품목손익등급별 측정값 합계가 가장 큰 그룹은 '측정값 합계가 가장 큰 품목손익등급 그룹'이다. "
        "이를 '최대 기여 등급'으로 재분류하거나 품목기여등급으로 바꾸어 표현하지 마라. "
...
def build_sims_analysis_profile(
    ...
    *,
    ...
    if bool(meta.get("current_table_followup")) or action_name.startswith("현재표"):
        ...
        labels = {_clean_text(column) for column in (() if columns is None else columns)}
        has_cost_metrics = bool(labels.intersection({"추정단위손익", "추정손익률", "추정기여금액"}))
        purpose = "승인된 제품 통계의 출고빈도·수명주기·품목등급 확인"
        focus = ["전체 제품수와 등급 분포", "신규품목 F와 무출고 X 구분", "가격자료의 기준월과 자료 부족", "품목손익등급", "품목기여등급", "샘플을 전체로 일반화 금지"]
        if has_cost_metrics:
            purpose += " 및 관리용 추정손익·추정기여 확인 (실제 매출 분석 아님)"
            focus[5:5] = ["추정단위손익·추정손익률", "추정기여금액 (매출액 아님)"]
