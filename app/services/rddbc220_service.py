# app/services/rddbc220_service.py
...
TABLE = "rddbc220"
log = logging.getLogger("ssai.sims.rddbc220")
BOOK_ALLOWED_PREFIX = ("0", "1", "2", "4", "5", "6", "7", "9")
...
# 날짜 필터는 date_from/date_to로 입력받지만, 월별 집계이므로 내부적으로는 month_from/month_to로 변환해서 처리한다.
...
def _base_filters(params: Dict[str, str]) -> str:
    ...
    _add_month6_filters(clauses, "Books_Stock.Rd22_Stock_YyMm", params)

    if clean_text(params.get("physic_cd")):
        add_filter(clauses, "Books_Stock.Rd22_Physic_Cd = %(physic_cd)s")
    ...
    if clean_text(params.get("ven_cd")):
        add_filter(clauses, "Books_Stock.Rd22_Ven_Cd = %(ven_cd)s")
    ...
    if clean_text(params.get("stock_cd")):
        ...
        add_filter(
            clauses,
            """(
                LTRIM(RTRIM(Books_Stock.Rd22_Stock_Cd)) = %(stock_cd)s
                OR LTRIM(RTRIM(Books_Stock.Rd22_Stock_Cd)) = %(stock_cd_trim)s
                OR RIGHT('00000' + LTRIM(RTRIM(Books_Stock.Rd22_Stock_Cd)), 6) = RIGHT('00000' + %(stock_cd)s, 6)
            )"""
        )

    if clean_text(params.get("stock_apply_cd")):
        add_filter(clauses, "Books_Stock.Rd22_Stock_Apply_Cd = %(stock_apply_cd)s")
    ...
    if clean_text(params.get("io_gu_prefix")):
        add_filter(clauses, "LEFT(Books_Stock.Rd22_Io_Gu, 1) = %(io_gu_prefix)s")

    stock_side = clean_text(params.get("stock_side")).lower()
    if stock_side in ("in", "입고", "매입"):
        add_filter(clauses, "LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('0','1','2','4')")
    elif stock_side in ("out", "출고", "매출"):
        add_filter(clauses, "LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('5','6','7','9')")
    ...
    if like_value(params.get("ven_group_nm")):
        ...
        add_filter(
            ...
            """EXISTS (
                ...
                WHERE V.Rd03_Ven_Cd = Books_Stock.Rd22_Ven_Cd
...
def get_rddbc220_df(params: Optional[Dict[str, str]] = None):
    params = coalesce_params(params)
    params["top"] = _monthly_query_top(params, default=200)

    allowed = ", ".join(f"'{x}'" for x in BOOK_ALLOWED_PREFIX)
    where_sql = _base_filters(params)

    sql = f"""
SELECT TOP (%(top)s)
    Books_Stock.Rd22_Stock_YyMm AS 재고년월,
    Books_Stock.Rd22_Physic_Cd AS 제품코드,
    Physic_Cd.Rd04_Physic_Nm AS 제품명,
    Books_Stock.Rd22_Stock_Cd AS 재고위치코드,
    Stock_Cd.Rd01_Hnm AS 재고위치,
    Books_Stock.Rd22_Ven_Cd AS 거래처코드,
    Ven_Cd.Rd03_Ven_Nm AS 거래처명,
    Books_Stock.Rd22_Stock_Apply_Cd AS 재고적용처코드,
    Stock_Apply.Rd03_Ven_Nm AS 재고적용처명,
    Books_Stock.Rd22_Io_Gu AS 입출고구분코드,
    Io_Gu.Rd01_Hnm AS 입출고구분,
    LEFT(Books_Stock.Rd22_Io_Gu, 1) AS 입출고구분앞자리,
    CASE
        WHEN LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('0','1','2','4') THEN '장부재고 입고집계대상'
        WHEN LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('5','6','7','9') THEN '장부재고 출고집계대상'
        ELSE '장부재고 규칙외'
    END AS 집계규칙판정,
    CASE WHEN LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ({allowed}) THEN 'Y' ELSE 'N' END AS 허용구분여부,
    Books_Stock.Rd22_In_Quantity AS 입고수량,
    Books_Stock.Rd22_In_Oquantity AS 입고할증수량,
    Books_Stock.Rd22_In_Supply_Price AS 입고공급가액,
    Books_Stock.Rd22_In_Tax_Price AS 입고세액,
    Books_Stock.Rd22_Out_Quantity AS 출고수량,
    Books_Stock.Rd22_Out_Oquantity AS 출고할증수량,
    Books_Stock.Rd22_Out_Supply_Price AS 출고공급가액,
    Books_Stock.Rd22_Out_Tax_Price AS 출고세액
FROM dbo.Rddbc220 AS Books_Stock
LEFT JOIN dbo.Rddbc040 AS Physic_Cd
    ON Books_Stock.Rd22_Physic_Cd = Physic_Cd.Rd04_Physic_Cd
LEFT JOIN dbo.Rddbc010 AS Stock_Cd
    ON Books_Stock.Rd22_Stock_Cd_Gcode = Stock_Cd.Rd01_Gcode
   AND Books_Stock.Rd22_Stock_Cd = Stock_Cd.Rd01_Tcode
LEFT JOIN dbo.Rddbc030 AS Ven_Cd
    ON Books_Stock.Rd22_Ven_Cd = Ven_Cd.Rd03_Ven_Cd
LEFT JOIN dbo.Rddbc030 AS Stock_Apply
    ON Books_Stock.Rd22_Stock_Apply_Cd = Stock_Apply.Rd03_Ven_Cd
LEFT JOIN dbo.Rddbc010 AS Io_Gu
    ON Books_Stock.Rd22_Io_Gu_Gcode = Io_Gu.Rd01_Gcode
   AND Books_Stock.Rd22_Io_Gu = Io_Gu.Rd01_Tcode
WHERE 1 = 1
{where_sql}
ORDER BY Books_Stock.Rd22_Stock_YyMm DESC, Books_Stock.Rd22_Physic_Cd, Books_Stock.Rd22_Ven_Cd
"""
    return query_to_df(sql, params)


def get_rddbc220_result(params: Optional[Dict[str, Any]] = None):
    params = coalesce_params(params)
    df = get_rddbc220_df(params)
    row_count = len(df) if df is not None else 0

    log.info("DBG get_rddbc220_result rows=%s", row_count)

    if df is None or df.empty:

        cond_text = _monthly_condition_text(params, basis_label="장부재고")
    ...
    try:
        ...
        analysis_summary = get_rddbc220_analysis_summary(analysis_params)
    except Exception:
        log.exception("get_rddbc220_analysis_summary failed")
        analysis_summary = {}
    ...
    condition_text = _monthly_condition_text(params, basis_label="장부재고")
...
def get_rddbc220_export_df(params: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    """
    장부재고월집계 CSV/Excel 다운로드용 전체 상세 DataFrame.
    """
    ...
    try:
        payload = build_result_payload(
            ...
            params=qparams,
            df=df,
            message=f"장부재고월집계 다운로드 {len(df):,}건",
        )
        out = payload.get("df_display")
        if isinstance(out, pd.DataFrame):
            return out
    except Exception:
        log.exception("get_rddbc220_export_df label/apply failed")

    return df


def get_rddbc220_analysis_summary(params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    장부재고월집계 LLM 분석용 전체 집계.
    """
...
WITH base AS (
    SELECT
        Books_Stock.Rd22_Stock_YyMm AS stock_yyyymm,
        NULLIF(LTRIM(RTRIM(Books_Stock.Rd22_Physic_Cd)), '') AS product_cd,
        COALESCE(NULLIF(LTRIM(RTRIM(Physic_Cd.Rd04_Physic_Nm)), ''), '(미지정)') AS product_nm,
        NULLIF(LTRIM(RTRIM(Books_Stock.Rd22_Stock_Cd)), '') AS stock_cd,
        COALESCE(NULLIF(LTRIM(RTRIM(Stock_Cd.Rd01_Hnm)), ''), '(미지정)') AS stock_nm,
        NULLIF(LTRIM(RTRIM(Books_Stock.Rd22_Ven_Cd)), '') AS vendor_cd,
        COALESCE(NULLIF(LTRIM(RTRIM(Ven_Cd.Rd03_Ven_Nm)), ''), '(미지정)') AS vendor_nm,
        NULLIF(LTRIM(RTRIM(Books_Stock.Rd22_Stock_Apply_Cd)), '') AS stock_apply_cd,
        COALESCE(NULLIF(LTRIM(RTRIM(Stock_Apply.Rd03_Ven_Nm)), ''), '(미지정)') AS stock_apply_nm,
        NULLIF(LTRIM(RTRIM(Books_Stock.Rd22_Io_Gu)), '') AS io_gu,
        COALESCE(NULLIF(LTRIM(RTRIM(Io_Gu.Rd01_Hnm)), ''), '(미지정)') AS io_gu_nm,
        CASE
            WHEN LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('0','1','2','4') THEN '입고집계대상'
            WHEN LEFT(Books_Stock.Rd22_Io_Gu, 1) IN ('5','6','7','9') THEN '출고집계대상'
            ELSE '규칙외'
        END AS side_nm,
        CAST(COALESCE(Books_Stock.Rd22_In_Quantity, 0) AS float) AS in_qty,
        CAST(COALESCE(Books_Stock.Rd22_In_Oquantity, 0) AS float) AS in_bonus_qty,
        CAST(COALESCE(Books_Stock.Rd22_In_Supply_Price, 0) AS float) AS in_supply,
        CAST(COALESCE(Books_Stock.Rd22_In_Tax_Price, 0) AS float) AS in_tax,
        CAST(COALESCE(Books_Stock.Rd22_Out_Quantity, 0) AS float) AS out_qty,
        CAST(COALESCE(Books_Stock.Rd22_Out_Oquantity, 0) AS float) AS out_bonus_qty,
        CAST(COALESCE(Books_Stock.Rd22_Out_Supply_Price, 0) AS float) AS out_supply,
        CAST(COALESCE(Books_Stock.Rd22_Out_Tax_Price, 0) AS float) AS out_tax
    FROM dbo.Rddbc220 AS Books_Stock
    LEFT JOIN dbo.Rddbc040 AS Physic_Cd
        ON Books_Stock.Rd22_Physic_Cd = Physic_Cd.Rd04_Physic_Cd
    LEFT JOIN dbo.Rddbc010 AS Stock_Cd
...
"""
    ...
    if overall_df.empty:
        return {"row_count_total": 0, "row_count": 0, "stock_basis": "장부재고"}
