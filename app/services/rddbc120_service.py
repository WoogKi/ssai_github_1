...
WITH trans_sum AS (
    SELECT
        ...
        Rd12_Ven_Cd,
        Rd12_Trans_Seq,
        SUM(COALESCE(Rd12_Fin_Supply_Price, Rd12_Supply_Price, 0)) AS Sum_Fin_Supply_Price,
        SUM(COALESCE(Rd12_Fin_Tax_Price, Rd12_Tax_Price, 0)) AS Sum_Fin_Tax_Price
    FROM dbo.Rddbc120
    WHERE NULLIF(LTRIM(RTRIM(Rd12_Trans_Seq)), '') IS NOT NULL
    GROUP BY Rd12_Trans_YyMmDd, Rd12_Ven_Cd, Rd12_Trans_Seq
),
tax_sum AS (
    SELECT
        Rd12_Tax_Di,
...
LEFT JOIN trans_sum AS TS
    ON Out_Put.Rd12_Trans_YyMmDd = TS.Rd12_Trans_YyMmDd
